    ...
    <section className="relative min-h-screen w-full overflow-hidden flex flex-col bg-[#1C1A18]">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute -top-40 -right-40 w-[700px] h-[700px] rounded-full"
          style={{ background: "radial-gradient(circle, hsl(20 45% 55% / 0.15), transparent 70%)" }} />
        <div className="absolute -bottom-40 -left-40 w-[600px] h-[600px] rounded-full"
          style={{ background: "radial-gradient(circle, hsl(20 35% 50% / 0.10), transparent 70%)" }} />
      </div>

      {/* ── Split layout: texto esquerda, imagem direita ── */}
      <div className="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-[1.1fr_1fr]
        gap-8 lg:gap-12 items-center
        px-6 sm:px-12 lg:px-20 pt-28 pb-10 max-w-[1400px] mx-auto w-full">
        ...
        <div className="flex flex-col justify-center">
          ...
          <h1 className="font-playfair font-bold text-white leading-[1.05] mb-7"
            ...
            <span className={`block ${r("delay-[80ms]")}`}>Consultoria</span>
          ...
          {/* Descriptor */}
          <p className={`text-white/55 text-base leading-relaxed max-w-[480px] mb-10 ${r("delay-[360ms]")}`}>
          ...
          {/* CTAs */}
          <div className={`flex flex-wrap gap-3 mb-12 ${r("delay-[440ms]")}`}>
            <a
          ...
          <div className={`flex items-stretch divide-x divide-white/10
            ...
            {heroStats.map(({ value, label }, i) => (
              <div key={i} className="flex-1 px-5 py-4 flex flex-col gap-1">
                <span className="font-playfair font-bold text-[hsl(20_45%_72%)] leading-none"
                  style={{ fontSize: "clamp(1.4rem, 2.2vw, 2rem)" }}>
                  {value}
                </span>
                <span className="text-[10px] font-medium uppercase tracking-wider text-white/35 leading-snug">
                  {label}
                </span>
              </div>
            ))}
        ...
        <div className={`relative ${r("delay-[300ms]")}`}>
          <div className="absolute -inset-4 rounded-[2rem] opacity-50 blur-2xl pointer-events-none"
            style={{ background: "radial-gradient(ellipse at center, hsl(20 45% 60% / 0.3), transparent 70%)" }} />

          <div className="relative rounded-[1.75rem] overflow-hidden
            border border-[hsl(20_35%_62%/0.2)]
            shadow-[0_30px_80px_rgba(0,0,0,0.5)]">
            <img
              src={conhecaServicos}
              alt="Conheça os serviços de consultoria alimentar da NP — boas práticas e segurança alimentar"
              className="w-full h-auto block"
              loading="eager"
            />
            <div className="absolute inset-0 pointer-events-none ring-1 ring-inset ring-white/10 rounded-[1.75rem]" />
          </div>

          {/* Floating tag */}
          <div className="absolute -bottom-4 -left-4 sm:-left-5
            bg-[#1C1A18] border border-[hsl(20_35%_62%/0.35)]
