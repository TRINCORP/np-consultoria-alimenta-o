...
const About = () => {
  ...
  const { ref: videoRef2, inView: videoInView } = useInView({
    threshold: 0.1,
    triggerOnce: true,
  });
  ...
  return (
    <>
      <SEO 
        title="Sobre Nós | NP Consultoria"
        description="Conheça a história da NP Consultoria e nossa missão de transformar a qualidade alimentar em resultados extraordinários."
        keywords="sobre NP Consultoria, história, missão, valores, consultoria alimentar"
      />
          ...
          <div className="container mx-auto px-4">
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              ...
              <div className={`order-2 lg:order-1 transition-all duration-1000 ${
                videoInView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'
              }`}>
                ...
                <div className="grid grid-cols-2 gap-4 mt-10">
                  {stats.slice(0, 2).map((stat, index) => (
                    <div 
                      key={index}
                      className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl p-5 transition-all duration-300 hover:border-primary/30"
                    >
              ...
              <div className={`order-1 lg:order-2 flex justify-center transition-all duration-1000 delay-200 ${
                videoInView ? 'opacity-100 translate-x-0 scale-100' : 'opacity-0 translate-x-10 scale-95'
              }`}>
                <div 
                ...
                >
                  ...
                  {/* Badge */}
                  <div className="absolute -right-4 top-1/4 bg-card border border-border/50 rounded-2xl px-4 py-3 shadow-xl transform rotate-3 group-hover:rotate-0 transition-transform duration-500">
                    <div className="flex items-center gap-2">
                      <Award className="w-5 h-5 text-primary" />
                      <span className="text-sm font-bold text-foreground">15+ Anos</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
              {stats.map((stat, index) => (
                <div 
                  key={index}
                  className="card-premium bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl p-6 text-center transition-all duration-300 hover:border-primary/30 hover:bg-card/80 group"
                >
          ...
          <div className="container mx-auto px-4 relative z-10">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
              {values.map((value, index) => (
                <div 
                  key={index}
                  className={`card-premium bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl p-8 text-center transition-all duration-700 hover:border-primary/30 hover:bg-card/80 group ${
                    valuesInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
